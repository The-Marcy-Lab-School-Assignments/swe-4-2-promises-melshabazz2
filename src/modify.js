...
const readFileSequentially = () => {
  const storyParts = []
  return fs.readFile(getPath('story-part-1.tx'), 'utf-8')
    .then((part1) => {
      storyParts.push(part1)
      return fs.readFile(getPath('story-part-2.tx'), 'utf-8')
    })

    .then((part2) => {
      storyParts.push(part2)
      return fs.readFile(getPath('story-part-3.tx'), 'utf-8')
    })

    .then((part3) => {
      storyParts.push(part3)
      return fs.readFile(getPath('story-part-4.tx'), 'utf-8')
    })
    .then((part4) => {
